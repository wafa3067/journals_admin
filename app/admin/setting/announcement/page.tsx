...
function Page() {
  ...
  const { getAnnounce, success, loading } = useAppSelector(
    (state) => state.getAnnouncement,
  );
  ...
  return (
    <div>
      ...
      {getAnnounce.length > 0 &&
        getAnnounce.map((news, index) => (
          <NewsCard
            key={index}
            date={news.date}
            title={news.title}
            ...
            deleteButton={true}
            deleteHandler={() => {
              dispatch(deleteAnnouncementData(news.id)).then(() => {
                dispatch(getAnnouncementData());
              });
            }}
          />
