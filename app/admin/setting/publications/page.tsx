...
} from "../../adminSlice/publications/addPublications";
import DeleteModel from "@/app/components/deleteModel";
import { Link } from "lucide-react";
import CustomText from "@/app/components/custom_text";
import { getPublicationInsights } from "../../adminSlice/publications/getPublications";
...
function Page() {
  ...
  const { publication } = useAppSelector((state) => state.getPublications);

  useEffect(() => {
    dispatch(getPublicationInsights());
  ...
  const EditHandler = () => {
    ...
    dispatch(
      ...
      dispatch(getPublicationInsights()); // Show the message from the response
    });
    setOpenModel(null);
  };
    ...
    <div className="min-h-screen  w-full">
      <header className="md:bg-[#00b4d8] p-5  flex justify-between items-center ">
        <h1 className="text-sm font-semibold flex items-center gap-2 text-black md:text-white">
          Publications Ethics Management
        </h1>
      </header>
      <div className="min-h-screen bg-gray-50 px-6 py-12 w-full">
        {openModel == "add" && (
          <div className="fixed inset-0  bg-transparent bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg w-96 max-h-full overflow-y-scroll">
              <p className="p-2">Add Publication Ethics </p>
              <div>
                <input
                  type="text"
                  name="title"
        ...
        {deleteContacts === "deleteEdit" && (
          <DeleteModel
            heading="Delete"
            handleSaveAbout={() => {
              dispatch(deletepublicationData(catId)).then(() => {
                dispatch(getPublicationInsights());
