...
import {
  addPartnershipPage,
  deletePartnership,
} from "../../adminSlice/partnership/partnershipAddSlice";
import { getPartnershipData } from "../../adminSlice/partnership/partnershipGetSlice";
...
function Page() {
  ...
  const { getPartnership, success, loading } = useAppSelector(
    (state) => state.getPartnership,
  );

  useEffect(() => {
    dispatch(getPartnershipData());
  }, [getPartnershipData]);
  ...
  return (
    <div>
      ...
      {openModal === "announcement" && (
        <div className="fixed inset-0 bg-[#00b4d8] bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            <h2 className="text-xl font-bold mb-4">Add Partnership</h2>
            <input
              type="file"
              onChange={(e) => {
      ...
      <div className="overflow-y-auto h-[calc(100vh-100px)]">
        {getPartnership.length > 0 &&
          getPartnership.map((Partnership, index) => (
            <NewsCard
              key={index}
              img={Partnership.image}
              date={Partnership.date}
              title={Partnership.title}
              description={Partnership.descriptions}
              link={Partnership.link}
              deleteButton={true}
              deleteHandler={() => {
                dispatch(deletePartnership(Partnership.id)).then(() => {
                  dispatch(getPartnershipData());
                });
              }}
            />
