...
import {
  addCareersPage,
  deleteCareers,
} from "../../adminSlice/careers/careersAddSlice";
import { getCareersData } from "../../adminSlice/careers/careersGetSlice";
...
function Page() {
  ...
  const { getCareers, success, loading } = useAppSelector(
    (state) => state.getCareers,
  );

  useEffect(() => {
    dispatch(getCareersData());
  }, [getCareersData]);
  ...
  return (
    <div>
      ...
      <div className="overflow-y-auto h-[calc(100vh-100px)]">
        {getCareers.length > 0 &&
          getCareers.map((Careers, index) => (
            <NewsCard
              key={index}
              img={Careers.image}
              date={Careers.date}
              title={Careers.title}
              description={Careers.descriptions}
              link={Careers.link}
              deleteButton={true}
              deleteHandler={() => {
                dispatch(deleteCareers(Careers.id)).then(() => {
                  dispatch(getCareersData());
                });
              }}
            />
