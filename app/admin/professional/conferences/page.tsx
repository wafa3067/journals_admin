...
import {
  addConferencesPage,
  deleteConferences,
} from "../../adminSlice/conferences/conferencesAddSlice";
import { getConferencesData } from "../../adminSlice/conferences/conferencesGetSlice";
...
function Page() {
  ...
  const { getConferences, success, loading } = useAppSelector(
    (state) => state.getConferences,
  );

  useEffect(() => {
    dispatch(getConferencesData());
  }, [getConferencesData]);
  ...
  return (
    <div>
      ...
      {openModal === "announcement" && (
        <div className="fixed inset-0 bg-[#00b4d8] bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            <h2 className="text-xl font-bold mb-4">Add Conferences</h2>
            <input
              type="file"
              onChange={(e) => {
      ...
      <div className="overflow-y-auto h-[calc(100vh-100px)]">
        {getConferences.length > 0 &&
          getConferences.map((Conferences, index) => (
            <NewsCard
              key={index}
              img={Conferences.image}
              date={Conferences.date}
              title={Conferences.title}
              description={Conferences.descriptions}
              link={Conferences.link}
              deleteButton={true}
              deleteHandler={() => {
                dispatch(deleteConferences(Conferences.id)).then(() => {
                  dispatch(getConferencesData());
                });
              }}
            />
